...
export default function ToggleDarkMode() {
    ...
    const toggleDarkMode = useCallback((prefersDark: boolean) => {
        if (prefersDark) {
            enableDarkMode();
        } else {
            disableDarkMode();
  ...
  return (
    <button
      aria-label={isDarkMode ? 'Перейти в світлий режим' : 'Перейти в темний режим'}
      className="ml-2 sm:ml-8"
      title={isDarkMode ? 'Перейти в світлий режим' : 'Перейти в темний режим'}
      onClick={handleClick}
    >
      {' '}
      {isDarkMode ? <img alt="Сонце" className={classes.icon} height="27" src={sunIcon.src} width="27" /> : <img alt="Місяць" className={classes.icon} height="27" src={moonIcon.src} width="27" />}
    </button>)
}