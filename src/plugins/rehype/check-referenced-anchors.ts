...
export default async function checkReferencedAnchors(tree: Root, file: VFile) {
  ...
  visit(tree, 'element', (node) => {
    ...
    if (baseUrl) {
      ...
      if (slug && hasPage(slug) && !hasAnchor(slug, anchor as string)) {
        console.warn(
          `Referenced anchor "${anchor}" not found in slug "${slug}"`,
        );
      }
    } else {
      if (!currentPageSlugs.has(anchor as string)) {
        console.warn(`Referenced anchor "${anchor}" not found in ${file.path}`);
      }
    }
  });
}
